"""
EXPERIMENT 4.1: Routing Swap on 4-Layer Transformer

Phase 4 of scaling validation: Prove G causality holds at deeper scales.

Protocol:
1. Train Model A (standard) and Model B (noisy) on Interleaved task (4-layer)
2. Both converge to >95% accuracy
3. Swap ALL QK parameters from B → A (frozen V/MLP)
4. Evaluate WITHOUT retraining

Hypothesis:
- 4-layer should exhibit same catastrophic failure as 1L/2L
- G×S decomposition is fundamental, not depth-specific
- May see even more pronounced crash due to more G-S entanglement

Success Criteria:
- Accuracy drops significantly (>50% from baseline)
- Pattern qualitatively matches 2-layer (G and S separability holds)
"""
...
def run_single_seed(
    ...
    n_steps: int = 20000,  # More steps for deeper model
    batch_size: int = 64,
    lr: float = 1e-3,
    seed: int = 42,
    device: str = 'cuda'
    ...
    """
    Run Experiment 4.1: Routing Swap on 4-Layer Models.
    ...
    print("\n" + "="*80)
    print(f"EXPERIMENT 4.1: ROUTING SWAP ON 4-LAYER MODELS (seed={seed})")
    print("="*80 + "\n")
    ...
    print("\n" + "-"*80)
    print("PHASE 1: Training Model A (Standard) - 4 Layers")
    print("-"*80 + "\n")
    ...
    print("\n" + "-"*80)
    print("PHASE 2: Training Model B (Noisy) - 4 Layers")
    print("-"*80 + "\n")
    ...
    print("\n" + "-"*80)
    print("PHASE 3: Full QK Swap (All 4 Layers) - Evaluate WITHOUT Retraining")
    print("-"*80 + "\n")
    ...
    # =========================================================================
    # LAYER-WISE SWAP ANALYSIS (4-layer specific)
    # =========================================================================

    print("\n" + "-"*80)
    print("PHASE 4: Layer-wise Swap Analysis (4-Layer Specific)")
    print("-"*80 + "\n")

    # Test swapping progressively more layers
    for n_swap_layers in [1, 2, 3, 4]:
        # Create fresh hybrid from A
        model_hybrid_partial = DeepTransformer(
            vocab_size=vocab_size,
            d_model=d_model,
            n_heads=n_heads,
            n_layers=n_layers,
        ...
        model_hybrid_partial.load_state_dict(model_a_state)
        model_hybrid_partial.to(device)

        # Swap first n_swap_layers
        layer_filter = list(range(n_swap_layers))
        swap_info_partial = swap_qk_parameters(
            model_hybrid_partial, model_b_temp,
            n_heads=None, layer_filter=layer_filter
        )

        partial_acc = validate(model_hybrid_partial, val_loader, device, vocab_size)
        partial_drop = (result_a['final_acc'] - partial_acc) / result_a['final_acc'] * 100

        print(f"Layers 0-{n_swap_layers-1} swapped: Acc={partial_acc:.3f} (drop={partial_drop:.1f}%)")

        results['swaps'].append({
            'type': 'partial_swap',
            'layers_swapped': layer_filter,
            'n_layers_swapped': n_swap_layers,
            'accuracy': partial_acc,
            'accuracy_drop_pct': partial_drop
        })
...
def run_experiment(
    ...
    print("\n" + "="*80)
    print(f"RUNNING 4-LAYER ROUTING SWAP EXPERIMENT (n={n_seeds} seeds)")
    print("="*80 + "\n")
    ...
    print("\n" + "="*80)
    print("EXPERIMENT 4.1 RESULTS (Multi-seed)")
    print("="*80 + "\n")
    ...
    print(f"Accuracy Drop (Full Swap): {m_drop:.1f}% ± {ci_drop:.1f}%")

    pass_count = sum(1 for r in all_results if r['success'])
    print(f"\nPass Rate (>50% drop): {pass_count}/{len(all_results)}")

    # Layer-wise analysis
    print("\n--- Layer-wise Swap Analysis (averaged) ---")
    for n_swap in [1, 2, 3, 4]:
        # Find the partial swap result for this layer count
        drops = []
        for r in all_results:
            for swap in r['swaps']:
                if swap.get('type') == 'partial_swap' and swap.get('n_layers_swapped') == n_swap:
                    drops.append(swap['accuracy_drop_pct'])
                    break
        if drops:
            m, ci = get_stats(drops)
            print(f"  {n_swap} layer(s) swapped: {m:.1f}% ± {ci:.1f}% drop")

    # Save results
    save_path = Path("clean_audit/data/exp_4_1_swap_4layer_results.json")
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 4.1: 4-Layer Routing Swap")
    ...
    n_steps = 4000 if args.quick_test else args.n_steps
