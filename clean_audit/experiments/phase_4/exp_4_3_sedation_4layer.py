"""
EXPERIMENT 4.3: Sedation Test (4-Layer)

Phase 4: Prove margin-as-budget story holds at 4 layers.

Protocol:
1. Train converged 4-layer model on Interleaved task
2. Clamp output amplitude to 60% of natural
3. Test clean (σ=0) vs noisy (σ=2.0) accuracy

Hypothesis:
- Same margin-as-budget phenomenon at 4 layers
- Prophylactic margin should be even more pronounced with depth
- Deeper models may have more sophisticated slack allocation
...
def measure_natural_amplitude(
    ...
    """Measure natural amplitude at each layer and final output."""
    model.eval()
    layer_amplitudes = {}
    final_amplitudes = []
    ...
    with torch.no_grad():
        for inputs, _ in dataloader:
            ...
            # Final residual amplitude
            if 'resid_final' in model.cache:
                resid = model.cache['resid_final']
                amp = resid.reshape(-1, resid.shape[-1]).norm(dim=-1).mean().item()
                final_amplitudes.append(amp)

            # Per-layer amplitudes
            for key, tensor in model.cache.items():
                if 'resid_post' in key and 'blocks' in key:
                    layer_idx = int(key.split('.')[1])
                    if layer_idx not in layer_amplitudes:
                        layer_amplitudes[layer_idx] = []
                    amp = tensor.reshape(-1, tensor.shape[-1]).norm(dim=-1).mean().item()
                    layer_amplitudes[layer_idx].append(amp)

    result = {
        'final': np.mean(final_amplitudes) if final_amplitudes else 0.0
    }
    for layer_idx, amps in layer_amplitudes.items():
        result[f'layer_{layer_idx}'] = np.mean(amps)
...
def run_single_seed(
    ...
    """
    Run Experiment 4.3: Sedation Test on 4-Layer Model.

    Protocol:
    1. Train 4-layer model to convergence
    ...
    print("\n" + "="*80)
    print(f"EXPERIMENT 4.3: SEDATION TEST (4-LAYER, seed={seed})")
    print("="*80 + "\n")
    ...
    print("-"*80)
    print("PHASE 1: Training 4-Layer Model")
    print("-"*80 + "\n")
    ...
    print("\n" + "-"*80)
    print("PHASE 2: Measuring Natural Amplitude (Per-Layer Analysis)")
    print("-"*80 + "\n")

    amplitude_info = measure_natural_amplitude(model, val_loader, device)
    natural_amplitude = amplitude_info['final']
    clamp_target = natural_amplitude * clamp_fraction

    print(f"Natural amplitude (final): {natural_amplitude:.2f}")
    print(f"Clamp target ({clamp_fraction:.0%}): {clamp_target:.2f}")

    # Per-layer amplitudes
    print("\nPer-layer amplitudes:")
    for key, val in sorted(amplitude_info.items()):
        if key != 'final':
            print(f"  {key}: {val:.2f}")
    ...
    natural_noise_drop = (baseline_clean - baseline_noisy) * 100
    sedated_noise_drop = (sedated_clean - sedated_noisy) * 100

    print(f"\nNatural noise robustness: {natural_noise_drop:.1f}% drop under noise")
    print(f"Sedated noise robustness: {sedated_noise_drop:.1f}% drop under noise")
    ...
    print(f"\nOverall: {'✓ MARGIN-AS-BUDGET HOLDS (4-LAYER)' if overall else '✗ INCONCLUSIVE'}")

    results['analysis'] = {
        'clean_degradation_pct': float(clean_degradation),
        'noisy_degradation_pct': float(noisy_degradation),
        'natural_noise_drop_pct': float(natural_noise_drop),
        'sedated_noise_drop_pct': float(sedated_noise_drop),
...
def run_experiment(
    ...
    print("\n" + "="*80)
    print(f"RUNNING 4-LAYER SEDATION EXPERIMENT (n={n_seeds} seeds)")
    print("="*80 + "\n")
    ...
    print("\n" + "="*80)
    print("EXPERIMENT 4.3 RESULTS (Multi-seed)")
    print("="*80 + "\n")
    ...
    # Per-layer amplitude analysis
    print("\n--- Per-Layer Amplitude (averaged) ---")
    for layer_idx in range(n_layers):
        key = f'layer_{layer_idx}'
        vals = [r['amplitude_info'].get(key, np.nan) for r in all_results]
        vals = [v for v in vals if not np.isnan(v)]
        if vals:
            m, ci = get_stats(vals)
            print(f"  Layer {layer_idx}: {m:.2f} ± {ci:.2f}")

    # Save
    save_path = Path("clean_audit/data/exp_4_3_sedation_4layer_results.json")
...
def main():
    parser = argparse.ArgumentParser(description="Experiment 4.3: 4-Layer Sedation Test")
